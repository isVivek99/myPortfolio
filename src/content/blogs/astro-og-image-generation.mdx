---
title: "Astro OG Image Generation on Cloudflare Workers (No Vercel, with workers-og)"
subheading: "Generate dynamic Open Graph images for an Astro site deployed on Cloudflare Workers using workers-og — no Vercel lock-in, no extra services."
slug: "astro-og-image-cloudflare-workers"
publishedAt: 2025-07-27
readingTimeInMins: 7
cover: ../featured_svgs/astro-og-image-generation.svg
coverAlt: "Astro blog post showing a dynamically generated Open Graph preview image"
tags: ["astro"]
description: "Generate dynamic OG images in Astro on Cloudflare Workers with workers-og — no Vercel lock-in. Full setup: Vite font plugin, SSR endpoint, nodejs_compat."
keywords: ["astro og image", "astro cloudflare workers", "workers-og", "astro open graph image", "astro og image generation", "dynamic og image astro"]
---
...
Hello folks, I am back with another blog. This time we're going to talk about **Astro OG image generation on Cloudflare Workers** — without Vercel and without vendor lock-in.

I've been using Astro for my personal portfolio for over a year. While working on SEO, I hit a wall trying to generate dynamic Open Graph images for blog posts. Almost every tutorial out there reaches for `vercel-og`, which is great if you're on Vercel — but useless if you've deployed your Astro site to **Cloudflare Workers**.

This guide walks through the setup I landed on: using [`workers-og`](https://github.com/kvnang/workers-og) for OG image generation in Astro, with SSR running on Cloudflare Workers. No serverless function detour, no Vercel migration, just an Astro endpoint that returns a PNG.
...
![Example of an Astro blog post Open Graph image preview on social media](../../../src/assets/blogs/astro-og-image-generation/img-2.png)
...
![Cloudflare Workers dashboard showing nodejs_compat compatibility flag setting for Astro](../../../src/assets/blogs/astro-og-image-generation/img-1.png)
...
<Note type="tip" title="Important Information">
  While the preview tools may display SVG images correctly, many social media
  platforms **do not support SVG format** for OG images. Stick with PNG format
  (which our implementation uses) for maximum compatibility across platforms
  like Twitter/X and Discord.
</Note>
### Real-World Testing
...
You can find the complete implementation of this OG image generation setup in my portfolio repository: [https://github.com/isVivek99/myPortfolio](https://github.com/isVivek99/myPortfolio)
